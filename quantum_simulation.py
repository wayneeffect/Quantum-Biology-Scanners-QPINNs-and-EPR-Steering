...
def quantum_simulation():
    ...
    c_ops = [
        np.sqrt(kappa) * qt.tensor(a, qt.qeye(N)),
    ...
    <p><strong>Entanglement Entropy (von Neumann):</strong> {entropy:.4f}</p>
    <p><strong>Diagonal Correlation XX:</strong> {corr_xx:.4f}</p>
    <p><strong>Diagonal Correlation PP:</strong> {corr_pp:.4f}</p>
    <hr>
    <p><small>Refresh page to re-run simulation • 6D+ Bivector + Polyharmonic Rotation Model</small></p>
    """
    return html
